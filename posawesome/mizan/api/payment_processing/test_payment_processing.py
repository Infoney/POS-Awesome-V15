...
def _install_framework_stubs():
    ...
    mpesa_module = types.ModuleType("posawesome.mizan.api.m_pesa")
    mpesa_module.submit_mpesa_payment = lambda *args, **kwargs: None
    sys.modules["posawesome.mizan.api.m_pesa"] = mpesa_module


def _install_package_stubs():
    package_paths = {
        "posawesome": REPO_ROOT / "posawesome",
        "posawesome.mizan": REPO_ROOT / "posawesome" / "posawesome",
        "posawesome.mizan.api": REPO_ROOT / "posawesome" / "posawesome" / "api",
        "posawesome.mizan.api.payment_processing": (
...
class TestPosPaymentProcessing(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        _load_module(
            "posawesome.mizan.api.payment_processing.utils",
            payment_processing_dir / "utils.py",
        )
        _load_module(
            "posawesome.mizan.api.payment_processing.creation",
            payment_processing_dir / "creation.py",
        )
        cls.data = _load_module(
            "posawesome.mizan.api.payment_processing.data",
            payment_processing_dir / "data.py",
        )
        cls.processor = _load_module(
            "posawesome.mizan.api.payment_processing.processor",
            payment_processing_dir / "processor.py",
        )
        cls.reconciliation = _load_module(
            "posawesome.mizan.api.payment_processing.reconciliation",
            payment_processing_dir / "reconciliation.py",
        )

    @patch("posawesome.mizan.api.payment_processing.processor.create_payment_entry")
    @patch("posawesome.mizan.api.payment_processing.processor.frappe")
    ...
    @patch(
        "posawesome.mizan.api.payment_processing.processor.find_payment_entries_by_client_request_id"
    )
    @patch("posawesome.mizan.api.payment_processing.processor.create_payment_entry")
    @patch("posawesome.mizan.api.payment_processing.processor.frappe")
    ...
    @patch(
        "posawesome.mizan.api.payment_processing.data.get_advance_payment_entries_for_regional"
    )
    @patch("posawesome.mizan.api.payment_processing.data.get_party_account")
    @patch("posawesome.mizan.api.payment_processing.data.frappe")
    ...
    @patch(
        "posawesome.mizan.api.payment_processing.data.get_erpnext_outstanding_invoices",
    ...
    @patch("posawesome.mizan.api.payment_processing.reconciliation.reconcile_against_document")
    @patch("posawesome.mizan.api.payment_processing.reconciliation.frappe")
    @patch("posawesome.mizan.api.payment_processing.reconciliation.get_unallocated_payments")
    @patch("posawesome.mizan.api.payment_processing.reconciliation.get_outstanding_invoices")
